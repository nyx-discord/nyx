...
const asyncBus = BasicEventBus<MyEventsArgs>.createAsync(
  bot,
  Symbol('myAsyncBus'), // Bus ID
);

// Uses a SyncEventDispatcher
const syncBus = BasicEventBus<MyEventsArgs>.createSync(
  bot,
...
const clientEventBus = BasicEventEmitterBus.createAsyncWithEmitter<MyEventsArgs>(
  bot,
...
const myEventEmitterBus = BasicEventEmitterBus.createAsyncWithEmitter<MyEventsArgs>(
  bot,
