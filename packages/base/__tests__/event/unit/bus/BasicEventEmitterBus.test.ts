...
describe('BasicEventEmitterBus', () => {
  ...
  describe('unsubscribe', () => {
    ...
    test('GIVEN a single subscriber WHEN unsubscribed THEN removes listener from emitter', async () => {
      const emitter = StubEventEmitter.create();
      const bus = createBus({ emitter });
      const subscriber = new MockEventSubscriber();
      await bus.subscribe(subscriber);
      ...
      expect(emitter.removeAllListeners).toHaveBeenCalledWith('test');
    });

    test('GIVEN two subscribers for same event WHEN one unsubscribes THEN emitter listener remains', async () => {
      const emitter = StubEventEmitter.create();
      const bus = createBus({ emitter });
      ...
      expect(emitter.removeAllListeners).not.toHaveBeenCalledWith('test');
    });
  });
  ...
  describe('clearSubscribers', () => {
    ...
    test('GIVEN subscribers WHEN cleared THEN removes listener from emitter', async () => {
      const emitter = StubEventEmitter.create();
      const bus = createBus({ emitter });
      const subscriber = new MockEventSubscriber();
      await bus.subscribe(subscriber);
