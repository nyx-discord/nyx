...
class MySession extends AbstractSession<void> {
  ...
  protected async handleButton(
    ...
    // You can also get the extra 'button' string here.
    const extra = meta.get(TypedKeys.CustomIdExtra); // 'button'
  }
...
const session = new MySession({ bot, id: sessionId, startInteraction });
...
A pagination session can serialize pagination information into customIds to create page switch components.
If you don't specify a page, it will be routed by default to the correspondent handler (`#handleSelectMenu()`, `#handleButton()` or `#handleModal()`).
...
class MyPaginationSession extends AbstractPaginationSession<void> {
  ...
  */
  protected override async updatePage(interaction: SessionUpdateInteraction) {
  ...
  protected createPage() {
    ...
    /** AbstractPaginationSessions have protected page manipulation methods. */
    ...
    /**
    * Builds an ActionRow with basic next and previous buttons.
    *
    * Similar to the example above, but with pages this.currentPage -+ 1
    */
...
const sessionId = 'mySessionId'; // Ideally randomly generated
const session = new MyPaginationSession({ bot, id: sessionId, startInteraction });
await bot.getSessionManager().start(session);
...
const sessionId = 'mySessionId'; // Ideally randomly generated
const session = new MyListPaginationSession({ bot, sessionId, interaction, items });
await bot.getSessionManager().start(session);
...
class Stage1 extends AbstractSessionStartStage<void> {
  ...
  public async onStart(
    ...
    const nextPageId = this.session.buildPageCustomId(nextPage);
  ...
  public async onSwitch(
    ...
    const buttonId = this.customId.buildCustomId('test');
    const button = new ButtonBuilder()
      .setStyle(1)
      .setLabel('test')
      .setCustomId(buttonId);
...
const sessionId = 'mySessionId'; // Ideally randomly generated
const session = new MyStageSession({ bot, id: sessionId, startInteraction });
await bot.getSessionManager().start(session);

// `await` for the session to end
await session.getEndPromise();

await startInteraction.editReply('Session ended!');
```
