...
class CommandCallSubscriber
  ...
  public handleEvent(
    ...
    commandMeta: MetaCollection,
  ) {
    const bot = TypedFields.Bot.get(commandMeta, true);
...
class InteractionCreateSubscriber
  ...
  public handleEvent(meta: MetaCollection, interaction: Interaction) {
    const bot = TypedFields.Bot.get(meta, true);
...
```ts
import { BasicEventBus, type MetaCollection } from '@nyx-discord/framework';
...
const validSubscriber = new SubscriberCallbackWrapper<TicketEventsArgs, 'ticketCreate'>(
  'ticketCreate',
  (meta: MetaCollection, createdTicket: Ticket) => {
    // Do something
  }
);
...
const invalidSubscriber = new SubscriberCallbackWrapper<TicketEventsArgs, 'ticketCreate'>(
  ...
  // error-next-line
  (meta: MetaCollection, createdTicket: number) => {
    // Do something
  }
);
