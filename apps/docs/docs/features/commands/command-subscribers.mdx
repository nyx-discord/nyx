...
```ts
import { CommandSubscriptionsContainer } from '@nyx-discord/core';
...
```ts
import { AbstractDJSClientSubscriber } from 'discord.js';
...
You can check the `DefaultCommandInteractionSubscriber` and `DefaultCommandAutocompleteSubscriber` to see how a
