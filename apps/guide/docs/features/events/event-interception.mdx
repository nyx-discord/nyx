...
From the `MetaCollection` passed on the subscriber's args you can get:
...
Also, since they have access to the subscriber that is being filtered and the `MetaCollection`, filters can share
