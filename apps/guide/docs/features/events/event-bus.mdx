...
Event buses extend the `Lockable` interface, so that they can be protected and unlocked.

If an event bus is protected, the `EventManager#removeEventBus()` method will throw an `LockedObjectError` when trying to
remove it.

You can set an event bus as protected by calling the `BasicEventBus#protect()` method (or overriding the `protected` property
when extending).
