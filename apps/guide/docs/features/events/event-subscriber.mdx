...
To get the bot that is calling an event, use the `TypedFields.Bot.get(meta)` method, using the meta passed for the event.
...
class MyEventSubscriber extends AbstractEventSubscriber<MyEventsArgs, 'someEvent'> {
  ...
  public handleEvent(meta: MetaCollection, ...args: MyEventsArgs['someEvent']) {
    // Null if you didn't pass a bot while creating your bus
    const bot = TypedFields.Bot.get(meta);
    if (!bot) {
...
class MyInterfaceEventSubscriber implements EventSubscriber {
  ...
  public handleEvent(meta: MetaCollection, ...args: unknown[]): void {
    // Null if you didn't pass a bot while creating your bus
    const bot = TypedFields.Bot.get(meta);
    if (!bot) {
      console.log('Hello world');
...
The first argument of the `EventSubscriber#handleEvent()` handler is an `MetaCollection` object, which is a
...
* Whether the event has been marked as handled by another subscriber, via the `TypedFields.EventHandled` field.

For more information about TypedFields, check the [📦 Meta Collections](/extra/meta) guide.

### ✅ Event handling marking

By default, when a subscriber marks an event as "handled" with `TypedFields.EventHandled.set(meta, true)`, it won't be
...
class HighestEventSubscriber extends AbstractEventSubscriber {
  ...
  public handleEvent(meta: MetaCollection): void {
    const bot = TypedFields.Bot.get(meta, true);
    bot.getLogger().log('Highest');
...
// highlight-next-line
  ...
  public handleEvent(meta: MetaCollection): void {
    const bot = TypedFields.Bot.get(meta, true);
    bot.getLogger().log('Lowest with ignoreHandled = false');
  }
}

await myBus
  .subscribe(
    new LowestEventSubscriber(),
    new HighestEventSubscriber(),
    new LowestNotIgnoredEventSubscriber(),
  );
 ...
 * Notice how Lowest wasn't logged, since Highest marked the event as handled.
 */
```
