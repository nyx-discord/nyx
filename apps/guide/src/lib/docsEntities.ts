...
type DocsEntitiesSource = 'auto' | 'local' | 'remote';

function getDocsEntitiesSource(): DocsEntitiesSource {
  const raw = process.env['NYX_DOCS_ENTITIES_SOURCE']?.trim().toLowerCase();

  if (!raw || raw === 'auto') return 'auto';
  if (raw === 'local' || raw === 'remote') return raw;

  console.warn(
    "'NYX_DOCS_ENTITIES_SOURCE' must be one of: auto, local, remote. Falling back to auto.",
  );
  return 'auto';
}

function shouldReadRemoteEntities(localExists: boolean): boolean {
  const source = getDocsEntitiesSource();
