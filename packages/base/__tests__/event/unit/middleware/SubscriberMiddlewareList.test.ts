import { TypedFields, EventSubscriberMiddlewareError } from '@nyx-discord/types';
...
describe('SubscriberMiddlewareList', () => {
  ...
  test('GIVEN a middleware that throws THEN wraps in EventSubscriberMiddlewareError', async () => {
    const list = SubscriberMiddlewareList.create();
    list.clear();

    const mockMiddleware = StubEventMiddleware.create();
    mockMiddleware.check = vi.fn().mockRejectedValue(new Error('Test throw'));
    list.add(mockMiddleware);

    const subscriber = new MockEventSubscriber();
    await expect(
      list.check(subscriber, StubMetadata.create())
    ).rejects.toThrowError(EventSubscriberMiddlewareError);
  });
