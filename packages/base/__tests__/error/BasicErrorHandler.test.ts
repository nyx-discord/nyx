import type { ErrorConsumer } from '@nyx-discord/types';
...
describe('BasicErrorHandler', () => {
  ...
  describe('consumer CRUD', () => {
    ...
    test('GIVEN an error class with registered consumer THEN hasConsumer returns true', () => {
      const handler = StubErrorHandler.create();
      handler.setConsumer(TestError, vi.fn<Consumer>());

      expect(handler.hasConsumer(TestError)).toBe(true);
      expect(handler.hasConsumer(Error)).toBe(false);
    });

    test('GIVEN an error instance with registered consumer THEN hasConsumer returns true', () => {
      const handler = StubErrorHandler.create();
      handler.setConsumer(TestError, vi.fn<Consumer>());

      expect(handler.hasConsumer(new TestError())).toBe(true);
      expect(handler.hasConsumer(new Error('unregistered'))).toBe(false);
    });
  });
  ...
  describe('fallback consumer', () => {
    ...
    test('GIVEN setFallbackConsumer called dynamically THEN unhandled errors dispatch to new fallback', async () => {
      const initialFallback = vi.fn<Consumer>();
      const handler = StubErrorHandler.create(initialFallback);
      ...
      expect(initialFallback).not.toHaveBeenCalled();
      expect(newFallback).toHaveBeenCalledWith(error, {} as Obj, []);
    });
  });
