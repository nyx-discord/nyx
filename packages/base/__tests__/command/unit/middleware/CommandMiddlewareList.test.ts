import { PriorityEnum, CommandMiddlewareError } from '@nyx-discord/types';
import {
  describe,
  expect,
  it,
...
describe('CommandMiddlewareList', () => {
  ...
  describe('Middleware execution', () => {
    ...
    test('GIVEN a middleware that throws THEN wraps in CommandMiddlewareError', async () => {
      const list = CommandMiddlewareList.create();
      list.clear();

      const mockMiddleware = StubMiddleware.create();
      (mockMiddleware.check as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('Test throw'));
      list.add(mockMiddleware);

      const command = new MockStandaloneCommand();
      await expect(
        list.check(command, {} as never, StubMetadata.create())
      ).rejects.toThrowError(CommandMiddlewareError);
    });
  });
