...
* `ScheduleExecutor`: Executes schedules, checking its `ScheduleMiddlewareLinkedList` and passing any errors to
its `ErrorHandler`.
...
<details>
  ...
  2. The `ScheduleExecutor` checks its `ScheduleMiddlewareLinkedList` which will allow or deny the execution.
...
participant ScheduleMiddlewareLinkedList
...
ScheduleExecutor -> ScheduleMiddlewareLinkedList: Check Middleware
ScheduleMiddlewareLinkedList -> ScheduleExecutor: Execution Allowed?
