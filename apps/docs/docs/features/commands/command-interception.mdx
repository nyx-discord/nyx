...
1. Add a `CommandMiddleware` to the `CommandMiddlewareLinkedList` in the `CommandExecutor` that handles
...
`CommandMiddlewares` are objects stored on a `CommandMiddlewareLinkedList`, which is held and called by
