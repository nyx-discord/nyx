...
> {
  ...
  public static readonly User = new SessionLimitScope((session) =>
    session.getUserId(),
  );

  /** Limits sessions per guild (by guild ID, or `'dm'` for DMs). */
  public static readonly Guild = new SessionLimitScope(
    (session) => session.getGuildId() ?? 'dm',
  );

  /** Limits sessions per channel (by channel ID, or `'unknown'` if missing). */
  public static readonly Channel = new SessionLimitScope((session) =>
    session.getChannelId(),
  );

  private readonly keyFn: (session: Session<unknown, Types>) => string;

  private constructor(keyFn: (session: Session<unknown, Types>) => string) {
    this.keyFn = keyFn;
  }
