...
1. Add a `ScheduleMiddleware` to the `ScheduleMiddlewareLinkedList` in the `ScheduleExecutor` that handles
...
`ScheduleMiddlewares` are objects stored on a `ScheduleMiddlewareLinkedList`, which is held and called by
